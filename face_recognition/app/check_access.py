    ...
    def recognition(self, img):
        ...
        for i in range(len(features)):
            ...
            if prob > self.threshold:
                ...
                names.append('unknow')
        return boxes, names, visitor_num
...
def predict(websocket):
    ...
    while True:
        ...
        if ret:
            start = time.time()
            boxes, names, visitor_num = predictor.recognition(img)
            visitor_total += visitor_num
            end_time = datetime.datetime.now()
            diff_time = (end_time - begin_time).seconds
            # 统计每个【3秒】内的人流量
            if diff_time > 3:
                time_HMS = end_time.strftime("%H:%M:%S")
                # visitor_total = random.randint(0, 100)
                data = [time_HMS, visitor_total]
            ...
            if boxes is not None:
                # 得到【最大人脸】的人名
                face_name = names[0]
                # 如果【最大人脸】的人名不是unknow，并且是未
                if face_name != "unknow":
                    predictor.draw_face(img, boxes, names)
                    print('预测的人脸位置：', boxes.astype('int32').tolist())
                    print('识别的人脸名称：', names)
                    print('总识别时间：%dms' % int((time.time() - start) * 1000))
                    sound_text = "%s，欢迎回校" % face_name

                    text_to_speech(sound_text)
                    # # 创建线程
                    # thread = threading.Thread(target=text_to_speech, args=(sound_text,))
                    # # 启动线程
                    # thread.start()
                else:
                    cv2.imshow("result", img)
