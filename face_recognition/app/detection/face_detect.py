...
# from detection.utils import generate_bbox, py_nms, convert_to_square
# from detection.utils import pad, calibrate_box, processed_image
...
class MTCNN:
    def __init__(self, model_path):
        ...
        # 获取P模型
        # a = os.path.join(model_path, '/PNet.pth')
        # self.pnet = torch.jit.load(os.path.join(model_path, '/PNet.pth'))
        self.pnet = torch.jit.load("save_model/mtcnn/PNet.pth", map_location=map_location)
        ...
        self.rnet = torch.jit.load("save_model/mtcnn/RNet.pth", map_location=map_location)
        self.rnet.to(self.device)
        self.softmax_r = torch.nn.Softmax(dim=-1)
        self.rnet.eval()

        # 获取R模型
        self.onet = torch.jit.load("save_model/mtcnn/ONet.pth", map_location=map_location)
